"""
Phase 1E Remediation — Real E2E Benchmark
Invokes research_graph directly to avoid HTTP auth complexity.
Same question as Phase 1D baseline.
"""
...
async def run() -> None:
    ...
    user_id = "31cc471e-a9c8-4ff2-9930-a771604b5e06"  # real user from existing sessions
    ...
    # Initialize Supabase clients (required when running outside FastAPI lifespan)
    from app.core.database import init_supabase_clients
    await init_supabase_clients()

    # Reset extraction semaphore (new event loop = need new semaphore)
    ...
    print(f"Report generated:        {report_obj is not None}")
    if report_obj:
        print(f"Report word count:       {report_obj.word_count}")
    ...
    print(f"{'Report generated':<28} {'yes':>12} {'yes' if report_obj else 'no':>12}")
