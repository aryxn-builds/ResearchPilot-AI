    ...
    if plan:
        for sq in plan.sub_questions:
            if sq.research_type == "web":
                sends.append(Send("web_research", {"sub_question": sq}))
        ...
        for sq in plan.sub_questions:
            for source in top_sources:
                if not source.is_flagged:
                    sends.append(Send("extract_evidence", {"sub_question": sq, "source": source}))
...
def verify_claims_loop(state: ResearchState) -> str:
    ...
    if iterations >= settings.RESEARCH_MAX_ITERATIONS_HARD_CAP:
        logger.warning("Critic loop reached hard cap limit", limit=settings.RESEARCH_MAX_ITERATIONS_HARD_CAP)
        return "write_report"
    ...
    needs_revision = any(
        c.verification_status in ("unverified", "contradicted")
        for c in critic_result.claims
    )
